...
from uploader.followup.runner import run_douyin_followup_daemon
from uploader.followup.runner import run_douyin_followup_once
from uploader.followup.store import FollowupStore
...
def _resolve_publish_time(publish_date: datetime | int) -> str:
    if isinstance(publish_date, datetime):
        return publish_date.isoformat(timespec="seconds")
    ...
    *,
    ...
    try:
        ...
        store.record_publish(
            ...
            tags=tags,
            publish_time=_resolve_publish_time(publish_date),
            post_id=str(publish_result.get("post_id", "") or ""),
            post_url=str(publish_result.get("post_url", "") or publish_result.get("redirect_url", "") or ""),
            metadata={
        ...
        print(f"Warning: failed to record publish event: {exc}", file=sys.stderr)


def _print_followup_report(report) -> None:
    print(
        "Followup report:",
        asdict(report),
    )
...
async def upload_video(request: DouyinVideoUploadRequest) -> Path:
    ...
    publish_result = await app.douyin_upload_video()
...
async def upload_note(request: DouyinNoteUploadRequest) -> Path:
    ...
    publish_result = await app.douyin_upload_note()
    _record_publish_event(
        platform="douyin",
        account_name=request.account_name,
        account_file=account_file,
...
async def run_followup_once(request: FollowupRunRequest):
    if request.platform != "douyin":
        raise RuntimeError(f"Unsupported followup platform: {request.platform}. Currently only douyin is available.")

    account_file = resolve_account_file(request.platform, request.account_name)
    if not account_file.exists():
        raise RuntimeError(
            f"Douyin account file is missing: {account_file}. Run `sau douyin login --account {request.account_name}` first."
        )

    return await run_douyin_followup_once(
        account_name=request.account_name,
        account_file=str(account_file),
        limit=request.limit,
        ...
        headless=request.headless,
        rules_file=str(request.rules_file) if request.rules_file else None,
        bootstrap_skip_existing=request.bootstrap_skip_existing,
        min_delay_seconds=request.min_delay_seconds,
        max_delay_seconds=request.max_delay_seconds,
        comment_page_url=request.comment_page_url,
    )


async def run_followup_daemon(request: FollowupDaemonRequest) -> int:
    if request.platform != "douyin":
        raise RuntimeError(f"Unsupported followup platform: {request.platform}. Currently only douyin is available.")
    ...
    return await run_douyin_followup_daemon(
        account_name=request.account_name,
        account_file=str(account_file),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    followup_parser = platform_parsers.add_parser("followup", help="Follow-up automation operations")
    followup_actions = followup_parser.add_subparsers(dest="action", required=True)

    followup_run_parser = followup_actions.add_parser("run", help="Run one follow-up cycle")
    followup_run_parser.add_argument("--platform", dest="target_platform", required=True, choices=["douyin"], help="Target followup platform")
    followup_run_parser.add_argument("--account", required=True, help="User-defined account_name")
    followup_run_parser.add_argument("--limit", type=int, default=20, help="Max comments fetched per cycle")
    followup_run_parser.add_argument("--max-replies", type=int, default=10, help="Max replies sent per cycle")
    followup_run_parser.add_argument("--since-hours", type=int, default=48, help="Only run if there are publish records within this window")
    followup_run_parser.add_argument("--dry-run", action="store_true", help="Only evaluate and log replies, without submitting")
    followup_run_parser.add_argument("--rules-file", type=existing_file_path, help="Optional JSON file for reply rules")
    followup_run_parser.add_argument("--reply-existing", action="store_true", help="On first run, also reply historical comments")
    followup_run_parser.add_argument("--min-delay-seconds", type=float, default=2.0, help="Min random delay between replies")
    followup_run_parser.add_argument("--max-delay-seconds", type=float, default=6.0, help="Max random delay between replies")
    followup_run_parser.add_argument("--comment-page-url", default="", help="Optional custom Douyin comment page URL")
    add_runtime_flags(followup_run_parser)

    followup_daemon_parser = followup_actions.add_parser("daemon", help="Run follow-up in loop mode")
    followup_daemon_parser.add_argument("--platform", dest="target_platform", required=True, choices=["douyin"], help="Target followup platform")
    followup_daemon_parser.add_argument("--account", required=True, help="User-defined account_name")
    followup_daemon_parser.add_argument("--limit", type=int, default=20, help="Max comments fetched per cycle")
    followup_daemon_parser.add_argument("--max-replies", type=int, default=10, help="Max replies sent per cycle")
    followup_daemon_parser.add_argument("--since-hours", type=int, default=48, help="Only run if there are publish records within this window")
    followup_daemon_parser.add_argument("--dry-run", action="store_true", help="Only evaluate and log replies, without submitting")
    followup_daemon_parser.add_argument("--rules-file", type=existing_file_path, help="Optional JSON file for reply rules")
    followup_daemon_parser.add_argument("--reply-existing", action="store_true", help="On first run, also reply historical comments")
    followup_daemon_parser.add_argument("--min-delay-seconds", type=float, default=2.0, help="Min random delay between replies")
    followup_daemon_parser.add_argument("--max-delay-seconds", type=float, default=6.0, help="Max random delay between replies")
    followup_daemon_parser.add_argument("--comment-page-url", default="", help="Optional custom Douyin comment page URL")
    followup_daemon_parser.add_argument("--interval-seconds", type=int, default=60, help="Loop interval in seconds")
    followup_daemon_parser.add_argument("--max-rounds", type=int, default=0, help="Stop after N rounds, 0 means infinite")
    add_runtime_flags(followup_daemon_parser)
    ...
    if args.platform == "followup":
        if args.action == "run":
            request = FollowupRunRequest(
                ...
                bootstrap_skip_existing=not args.reply_existing,
                min_delay_seconds=args.min_delay_seconds,
                max_delay_seconds=args.max_delay_seconds,
                comment_page_url=args.comment_page_url or None,
            )
            report = await run_followup_once(request)
            _print_followup_report(report)
            return 0 if report.failed == 0 else 1

        if args.action == "daemon":
            request = FollowupDaemonRequest(
                platform=args.target_platform,
                account_name=args.account,
                limit=args.limit,
                since_hours=args.since_hours,
            ...
            rounds = await run_followup_daemon(request)
            print(f"Followup daemon finished after {rounds} rounds")
            return 0

        raise RuntimeError(f"Unsupported Followup action: {args.action}")
