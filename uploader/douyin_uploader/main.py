...
def _extract_post_reference(url: str) -> tuple[str, str]:
    ...
    for key in ("item_id", "aweme_id", "group_id", "video_id", "note_id", "modal_id"):
        value = query.get(key, [None])[0]
        if value:
            return str(value), cleaned_url

    path_match = re.search(r"/(?:video|note|item|aweme)/(\d+)", parsed.path)
    if path_match:
        return path_match.group(1), cleaned_url

    number_match = re.search(r"(\d{8,})", parsed.path)
    if number_match:
