...
def _extract_post_reference(url: str) -> tuple[str, str]:
    ...
    for key in ("noteId", "note_id", "item_id", "video_id", "post_id"):
        value = query.get(key, [None])[0]
        if value:
            return str(value), cleaned_url

    path_match = re.search(r"/(?:explore|video|item|note)/([a-zA-Z0-9]+)", parsed.path)
    if path_match:
        return path_match.group(1), cleaned_url

    number_match = re.search(r"([a-zA-Z0-9]{8,})", parsed.path)
    if number_match:
        return number_match.group(1), cleaned_url
...
class XiaoHongShuVideo(XiaoHongShuBaseUploader):
    ...
    async def upload(self, playwright: Playwright) -> dict:
        ...
        try:
            ...
            post_id, post_url = _extract_post_reference(page.url)
            publish_result = {
                "success": True,
                "post_id": post_id,
                "post_url": post_url,
